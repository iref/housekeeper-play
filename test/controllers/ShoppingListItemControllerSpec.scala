...
class ShoppingListItemControllerSpec extends PlaySpecification with Mockito {
  ...
  def withShoppingListRepository[T: AsResult](f: (ShoppingListRepository, Application) => T): Result = {
    WithControllers.running((components, app) => f(components.shoppingListRepository, app))
  }

  def withShoppingListItemRepository[T: AsResult](f: (ShoppingListItemRepository, Application) => T): Result = {
    WithControllers.running((components, app) => f(components.shoppingListItemRepository, app))
  }
  ...
  "#save" should {
    ...
    "save valid item" in withShoppingListItemRepository { (shoppingListItemRepository, app) =>
      ...
      shoppingListItemRepository.add(1, expectedShoppingListItem) returns
  ...
  "#remove" should {
    "remove item from repository" in withShoppingListItemRepository { (shoppingListItemRepository, app) =>
      // given
      shoppingListItemRepository.remove(1) returns Future.successful(1)
      ...
      there was one(shoppingListItemRepository).remove(1)
    }
  ...
  "#edit" should {
    "render editItem template" in withShoppingListItemRepository { (shoppingListItemRepository, app) =>
      ...
      shoppingListItemRepository.find(1) returns Future.successful(Some(item))
  ...
  "#update" should {
    "not update item without name" in WithControllers.running { (_, app) =>
      // given
    ...
    "not update item without quantity" in WithControllers.running { (_, app) =>
      // given
    ...
    "not update item with negative quantity" in WithControllers.running { (_, app) =>
      // given
    ...
    "not update item with negative price for one" in WithControllers.running { (_, app) =>
      // given
    ...
    "update existing item" in withShoppingListItemRepository { (shoppingListItemRepository, app) =>
      ...
      shoppingListItemRepository.update(updated) returns (Future.successful(1))
      ...
      there was one(shoppingListItemRepository).update(updated)
    }
  }
